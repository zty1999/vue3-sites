<template>
<div id="earth-cantainer" ref="earthRef"></div>
</template>
<script lang="ts" setup>import { Earth } from './Earth';
  const earthRef = ref<HTMLElement>()
  // 城市数据
  onMounted(()=>{
    let cityList = {
    ...
    if(!earthRef || !earthRef.value){
      console.error('未提供渲染容器')
    }
    // 地球创建 加载  Earth('容器','城市数据','飞线关系','其它视图参数配置')
    let e = new Earth(earthRef.value!,cityList,bizLines,{autoRotate:true,zoomChina:true,starBackground:true});
